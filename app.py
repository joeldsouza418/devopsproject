...
<body class="bg-light">
    <div class="container mt-5">
        ...
        <div class="card shadow-lg p-4" style="border-radius:20px;">
            <h2 class="text-center text-primary mb-4">Hello from DevOps Pipeline 🚀</h2>

            <h4 class="text-center mb-4">Set Your Exam Reminder</h4>
            ...
            <form method="POST">
                ...
                <button class="btn btn-primary w-100 mt-2">Save Reminder</button>
            </form>
...
def send_email(to_email, subject, body):
    sender = "joeld7569@gmail.com"
    password = "pkgs xbku mdgr ubnc"
...
def home():
    ...
    if request.method == "POST":
        ...
        body = f"Hello {name}, your exam is scheduled on {exam_date}. This is your reminder!"
        send_email(email, "Exam Reminder Saved ✔", body)

        msg = "Reminder saved & email sent instantly!"

    return render_template_string(HTML_PAGE, msg=msg)

# Daily scheduled check (keep it)
def check_exam_reminders():
    today = datetime.now().strftime("%Y-%m-%d")
    reminders = load_reminders()
    remaining = []
    ...
    for r in reminders:
        if r["exam_date"] == today:
            body = f"Hello {r['name']}, this is a reminder — your exam is today!"
            send_email(r["email"], "Today is Your Exam", body)
        else:
            remaining.append(r)
